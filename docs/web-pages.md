...
關鍵字：trustees, schemes, funds, funds/detail, 受託人, 計劃, 基金, 基金詳情頁, 收藏, 書籤, bookmark, favourites, SEO, TrusteeSchemeSelector, FundDetailView, FundChart, FundListWithBookmarks, FavouritesView, BookmarkButton, 本月表現最佳基金, top10_funds_this_month, 示範數據, 大市指數, 基金一覽, 搜尋基金, fund search, filter fund name.
...
| React 組件 | `TrusteeSchemeSelector.tsx`、`SchemeFundList.tsx`（計劃基金列表 + 右側書籤 + 詳情連結）、`FundDetailView.tsx`（基金詳情 + 書籤）、`FundChart.tsx`、`FundListWithBookmarks.tsx`（基金一覽 + 標題列右側搜尋輸入與圖示、即時依中文/英文基金名篩選 + 右側書籤）、`FavouritesView.tsx`（我的收藏：已收藏基金列表連結至詳情 + 右側書籤）、`BookmarkButton.tsx`、`FundDetailBlock.tsx`（單一基金詳情區塊） |
...
- **全部基金列表**：`/funds/` 基金一覽以「中文第一行、英文第二行」顯示（有 `zh` 時），每行右側有書籤按鈕；每項可點進 `/funds/detail?fund={名稱}`。資料由 Astro 從 `fund_price_scheme.json` 建 `allFunds`（以 fund 去重、取首次 zh）。**基金一覽**標題列右側有搜尋輸入框與搜尋圖示，可依中文或英文基金名稱即時篩選列表（client 端 `FundListWithBookmarks` 內 `useState` + `useMemo` 過濾）。
